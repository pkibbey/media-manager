...
import { retryFailedExifFiles } from '@/app/actions/exif';
...
import type { ExifFailedData } from './exif-failed-processor';
import { ProcessingTimeEstimator } from './processing-time-estimator';

interface ExifFailedProcessorClientProps {
  initialData: ExifFailedData;
  onRefresh: () => void;
}

export default function ExifFailedProcessorClient({
  initialData,
  onRefresh,
}: ExifFailedProcessorClientProps) {
  ...
  useEffect(() => {
    if (initialData.files.length === 0) return;
  ...
  const largeFileCount = initialData.files.filter(
    (file) =>
      selected[file.id] &&
      file?.size_bytes &&
        ...
        <>
          ...
          <div className="border rounded-md p-4 bg-background">
            ...
            <div className="space-y-2">
              {initialData.errorCategories.map((category) => (
                <div key={category.type} className="text-sm">
                  ...
                  <div className="w-full bg-secondary rounded-full h-1.5 mt-1 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(category.count / initialData.files.length) * 100}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <Label htmlFor="selectAll" className="text-sm">
                Select All ({initialData.files.length} files)
              </Label>
            </div>
