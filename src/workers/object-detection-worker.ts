 ...
 */
const workerProcessor = async (job: Job<ObjectDetectionJobData>) => {
  ...
  try {
    ...
    if (mediaUpdateError) {
      ...
      console.log(
        `[Worker] Successfully marked media ID ${mediaId} as basic processed.`,
      );
    }

    return { mediaId, predictionsCount: predictions.length };
  } catch (error) {
    const errorMessage =
...
worker.on('completed', (job: Job<ObjectDetectionJobData>, result: any) => {
  console.log(
    `[Worker] Job ${job.id} (Media ID: ${job.data.id}) completed. Detected ${result.predictionsCount} objects.`,
  );
});
