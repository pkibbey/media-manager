-- This migration updates the get_exif_statistics function to correctly
-- count unprocessed items when there are no processing_states entries yet
...
BEGIN
  RETURN QUERY
  WITH 
    -- Find all media items with compatible extensions
    compatible_media AS (
      SELECT mi.id
      FROM media_items mi
