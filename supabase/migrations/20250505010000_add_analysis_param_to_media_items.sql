-- First drop the existing get_media_items function
DROP FUNCTION IF EXISTS public.get_media_items;

-- Add p_has_analysis parameter to get_media_items function
CREATE OR REPLACE FUNCTION public.get_media_items(
  ...
  p_has_analysis TEXT DEFAULT 'all',
...
BEGIN
  ...
  IF NOT p_include_hidden THEN
    v_filter_conditions := v_filter_conditions || ' AND (media_items.is_hidden = FALSE OR media_items.is_hidden IS NULL)';
  END IF;
  
  IF NOT p_include_deleted THEN
    v_filter_conditions := v_filter_conditions || ' AND (media_items.is_deleted = FALSE OR media_items.is_deleted IS NULL)';
  END IF;
  ...
  IF p_has_analysis IS NOT NULL AND p_has_analysis <> 'all' THEN
    IF p_has_analysis = 'yes' THEN
      v_filter_conditions := v_filter_conditions || ' AND analysis_data IS NOT NULL AND analysis_data::text <> ''{}''';
    ELSE
      v_filter_conditions := v_filter_conditions || ' AND (analysis_data IS NULL OR analysis_data::text = ''{}'')';
    END IF;
  END IF;
